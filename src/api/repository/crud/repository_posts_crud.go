...
func (r *repositoryPostsCRUD) FindById(pid uint64) (models.Post, error){
	...
	go func(ch chan <- bool){
		...
		err = r.db.Debug().Model(&models.User{}).Where("id=?", pid).Take(&post).Error
		if err != nil {
			ch <- false
			return
		}
...
func (r *repositoryPostsCRUD) Update(pid uint64, post models.Post) (int64, error){
	...
	go func(ch chan <- bool){
		...
		rs = r.db.Debug().Model(&models.User{}).Where("id=?", pid).Take(&models.Post{}).UpdateColumns(
...
func (r *repositoryPostsCRUD) Delete(pid uint64, uid uint32) (int64, error){
	...
	go func(ch chan <- bool){
		...
		rs = r.db.Debug().Model(&models.User{}).Where("id=? and author_id=?", pid, uid).Take(&models.Post{}).Delete(&models.Post{})
		ch <- true
	}(done)
	if channels.OK(done){
		if rs.Error!=nil{
